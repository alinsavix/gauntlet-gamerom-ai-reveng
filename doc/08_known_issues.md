...
- **Verified:** the fine ROM-byte audit now classifies every byte in the mixed
  ...
  entries), closing the then-current checked union at 321/321. The later
  `dragon_head_pose_update` entry raised the current union to 322/322. It also bounded the remaining
...
- **Verified:** an independent linear decode of the 34 proven executable
  ...
  explicit/indexed-base RAM literals as the 322-entry callable-body report.
...
- **Contradicted and corrected:** 0x4D1A4 is `secret_check_winner`, not `secret_continue_disallowed`. Its sole caller is the secret branch of `show_level_end_bonus_screen`; -1 selects the `5,000 × COINS` award and zero selects `NO BONUS !!`.
...
- **Verified:** the seven-row thief-state/secret-room catalog closed the original 294-entry set. Later closure work added 28 shipped veneers, pointer-installed leaves, interior entries, and dormant/legacy entries; the current machine union proves all 322 indexed entries have arguments, return behavior, purpose, and any exceptional convention recorded in a body-checked catalog.
