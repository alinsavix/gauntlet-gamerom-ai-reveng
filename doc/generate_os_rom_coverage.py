...
def generated(root: Path) -> tuple[list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]]]:
    ...
    for filename in ("os_control_targets.csv", "control_targets.csv"):
        for row in read_csv(doc / filename):
    ...
    callable_rows = read_csv(doc / "os_callable_contracts.csv")
    active_entries = [
    ...
    legacy_entries = [
        (int(row["address"], 16), row["name"])
        for row in read_csv(doc / "os_legacy_module_contracts.csv")
    ]
    residue_entries = [
        (int(row["address"], 16), row["name"])
        for row in read_csv(doc / "os_residue_contracts.csv")
    ]
