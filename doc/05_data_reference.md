...
| 0x904022 | 2 B | `potion_player` | Potion owner plus trigger flags: player index in bits 0-1, player-shot-triggered bit 2; the matrix consumer adds enhanced-magic bit 3 locally. Monster/dragon shots branch before this word is written. |
...
| 0x904AD6 | 2 B × 4 | `player_pending_damage` | Per-player damage accumulated since the periodic update. At a sample, health <500 and `pending*4 > health` triggers the low-health path. Values >20 add to `player_damage_total`; values ≤20 instead permit the commentary-average test. Cleared when an ordinary sample reloads, but retained through the negative commentary cooldown. |
| 0x904ADE | 2 B × 4 | `player_damage_total` | Per-player cumulative high-damage samples, saturated at 0x7D00. On a quiet sample it is divided signed by `player_damage_sample_count`: average >80 after count >3 selects speech 0x60/0x5F; average <60 clears the averaging window. |
| 0x904AE6 | 2 B × 4 | `player_damage_sample_timer` | Per-player signed timer. Positive values count down from 60 to a sample. Damage commentary writes -600; negative values count upward, and the zero crossing clears the sample count before 60-frame sampling resumes. |
| 0x904AEE | 2 B × 4 | `player_damage_sample_count` | Completed positive sampling intervals. Incremented before each sample test; commentary requires it to be strictly greater than 3. Cleared for average <60 or when the -600 commentary cooldown returns to zero. |
...
| 0x57952 | 16 B | `player_coin_sound_ids` — four character-indexed longword join-effect IDs `{0x09,0x0A,0x0B,0x0C}` read by `player_join_finalize` at 0x48AB2. Despite the historical table name, this is distinct from the player-position coin-slot commands 0x22–0x25. |
...
| 0x59B7C | 48 B | `powerup_speech_ids` — 12 longword speech-command IDs parallel to `powerup_bit_masks`: `{0x8F,0x90,0x91,0x92,0x93,0x94,0x8E,0xD1,0xCF,0xD0,0,0}`. For a nonzero entry, `player_give_item_with_message` uses `sound_speech_play` for player name, 0x8D (`NOW HAS`), and this entry; reduced text skips the first two, speech-disable gates every emitted phrase, and zero suppresses all three. |
