...
| 0x4BE24 | `level_splash` | Draws the new-level splash: branches on `mazenum_current` (0x904000) against 0x68/0x73 and on `levelnum_current` (0x904004); level 1 walks a linked list of text records from the pointer at 0x59B2E following the next field at `0xA(A2)`. When `secret_need_hint` is set, 0x4C04E–0x4C108 writes the secret-hint header and the eligible selected maze-header objective (or one of 17 random hints), then clears the latch. Other levels index the string tables 0x597D8/0x59736/0x5975E with `getrandom`, rendering each record through OS `display_text` (A2 = 0x142) and announcing it via `sound_speech_play`. |
