...
| 0x45C00 | `main_open_doors` | Advances up to 8 live `door_endpoint_pos` fronts, dispatching directions 0–3 through 0x45C46. One next-cell probe per channel: up/down accept 0x9D7C–0x9DAC; left/right accept 0x9D3C–0x9D6E. Junction pictures 0x9D18–0x9D38 always turn left (0→3, 1→0, 2→1, 3→2), not according to connectivity. Removes through `moblist_remove_and_clear` without restamping neighbors; mismatches park the front. Horizontal steps wrap within a row; Up rejects row zero, Down rejects ≥0x400. |
...
| 0x46FEA | `main_handle_potions` | Per-frame potion handling for players 0–3: skips empty `active_mob_ids` slots; normal play matches code 0x1C from `debounce_shift_magic` (0x905F58), while nonzero game modes synthesize that code by testing active-low Magic bit 0 directly at the current replayed `demo_ptr`; plays deny sound 0x44 when `player_potionsnum` (0x904055) is zero or 0x904000 ≥ 0x73, otherwise spends one potion, records `potion_player` (0x904022), plays 0x1D, and applies the effect through `player_inv_update`, `dialog_first_encounter` and `dragon_any_segment_near_screen` |
...
| 0x49034 | `main_move_monsters` | Per-frame monster movement/AI dispatch; computes the wrapped SLIP stop/start entries from `(pf_vscroll_lo + 0x118) & 0x1F0` and `(pf_vscroll_lo - 8) & 0x1F0` through the biased table base at 0x905F82 before calling `monsters_everything` |
...
| 0x4E8DC | `main_thief_anim` | Per-frame thief movement/animation state machine; increments `thief_stolen_item` as the shared escape/fight/compact animation counter, including the 16-frame monster-fight dwell |
...
| 0x4BE24 | `level_splash` | Draws the new-level splash: writes the hidden-potion and LFLAG ShotStun/ShotHurt/PlayerOffscreen/InvisibleAllWalls/InvisibleTrapWalls/ExitMoves records at 0x598B8–0x5999B with one shared speech latch and exact bounded draws; level 1 walks the linked record at 0x59B2E. When `secret_need_hint` is set, 0x4C04E–0x4C108 writes the secret-hint header and eligible selected maze-header objective (or one of 17 random hints), then clears the latch. Other ordinary levels use `getrandom(9)` to index the paired string tables 0x59736/0x5975E unless reduced-text mode is set. |
| 0x438AE | `maze_new_level_setup` | New-level initialization hub: clears slow motion at 0x438C2, writes -1 to the thief arrival timer/victim and zero to its current position at 0x438CA–0x438D6, then selects/decodes the maze, builds object and exit tables, and for secret mazes uses challenge target table 0x57056 to replace matching generators with exits, clear the other eligible generators, and replace ordinary monsters with typed hidden potions |
...
| 0x5E80C | `maze_convert_walls_to_exits` | No-argument routine (calls `mob_place_tile` at 0x5E852) invoked at the escape timeout: scans the wall markers, converts eligible ones — marker 0x20F6, or the 0x8000 bit set with the forcefield type 0x3F excluded at 0x5E844 — to unlinked type-0x10 markers with picture 0x8001, and returns `D0.l` = 1 when at least one conversion occurred |
...
| 0x4EE7A | `thief_move_engine` | Applies one thief move step from the flag word at `0xA(A6)` and the 0x80-biased H/V deltas: proposes each native axis word, calls `thief_probe_axis` with callbacks 0x408A0/0x406B6/0x40732/0x4083A, handles walls/players/items, applies the directional arms' one-pixel perpendicular response to an offset high-bit wall flank, derives the destination slot with the +12 H/+8 V body bias, and relinks through `moblist_replace` when empty; returns `thief_collision_direction_code` plus the movement-result flag |
