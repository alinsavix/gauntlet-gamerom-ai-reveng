...
*All 116 mazes (numbers 0–115) with bank assignments, ROM offsets, secret tricks, random food counts, and base flags.*
...
2. Reads the **pointer table** starting at the address stored at `0x38000` (which points to `0x3800C`). Each entry is a longword pointing to the maze data within the 32 KB slapstic ROM address space (`0x38000–0x3FFFF`).

---

## 2. Slapstic ROM Bank Layout

The slapstic ROM (row9.bin, 32 KB) is divided into 4 banks of 8 KB each:
...
The four `level_flags` bytes are assembled by `maze_load_pickup_config` (0x436FE) into the 32-bit `ram.maze_pickup_config` word (read by `maze_addrandompickups` and gameplay systems).
