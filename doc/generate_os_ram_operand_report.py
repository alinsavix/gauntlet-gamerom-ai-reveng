...
def generated(root: Path) -> tuple[list[dict[str, str]], list[dict[str, str]], list[dict[str, str]]]:
    doc = root / "doc"
    implementations = [row for row in read_csv(doc / "os_callable_contracts.csv") if row["entry_kind"] == "implementation"]
...
def main() -> None:
    ...
    rows, non_address_rows, failures = generated(here.parent)
