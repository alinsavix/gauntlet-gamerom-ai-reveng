...
CONTRACTS = (
    ...
    (0x8702, 0x89AA, "legacy_monster_choose_direction", "D2.w object byte offset; inherited A2-A5 VRAM/state bases; one caller word at +0x04", "updates object direction/path state; condition codes", "register worker reached only from 0x8000; builds its own A6 frame with link.w a6,#-0xC at 0x8716 and unlk at 0x8998, so A6 is not an inherited base", "Chooses a direction from nearby player positions, validates terrain interactions, and schedules the legacy move/path operation", "41f90090", "bsr.w 0x89e6"),
    (0x89AA, 0x89E6, "legacy_four_cell_occupied_test", "D4.w first cell byte offset; inherited A2 and the alternate occupancy base 0x90487A", "condition codes from the stopping tst.w; D4 advanced by up to six", "register leaf", "Tests four consecutive cells against the primary and alternate occupancy arrays", "41f90090", "tst.w (a2, d4.w)"),
    (0x89E6, 0x8A12, "legacy_position_in_active_bounds", "D4.b horizontal; D5.b vertical", "D4.l=-1 inside the active window, 0 outside; D5 clobbered by the origin subtraction", "register leaf", "Checks a wrapped position against the legacy active-screen bounds", "98390090", "moveq 0xff, d4"),
    ...
    (0x8A84, 0x8AE8, "legacy_moblist_add_object", "slot index word; picture word; H word; V word; link-lo field word; link-hi field word", "void", "normal stack ABI; module has no Gauntlet II caller", "Populates one MOB slot and links it: doubles the slot index into a byte offset, installs the five array bases, calls legacy_moblist_insert, writes picture/H/V to (A2/A3/A4,D1), then merges two caller-supplied six-bit fields into the top of the A5/A6 link words by masking each with 0x03FF and OR-ing the argument shifted left by ten", "48e73f3e", "bsr.b 0x8ae8"),
    ...
    (0x8B9E, 0x8BD4, "legacy_move_mob_slot_entry", "source slot word; destination slot word", "void", "normal stack ABI; module has no Gauntlet II caller", "Stack entry for the slot mover: doubles the source into D2 and destination into D1, installs the five MOB array bases, and calls legacy_move_mob_slot", "48e73f3e", "bsr.b 0x8c36"),
    (0x8BD4, 0x8C04, "legacy_moblist_remove_and_clear_entry", "slot word", "void", "normal stack ABI; module has no Gauntlet II caller", "Stack entry for the destructive remove: doubles the slot argument into D2, installs the five MOB array bases, and calls legacy_moblist_remove_and_clear", "48e73f3e", "bsr.b 0x8c70"),
    (0x8C04, 0x8C36, "legacy_moblist_unlink_entry", "slot word", "void", "normal stack ABI; module has no Gauntlet II caller", "Stack entry for the non-destructive unlink: doubles the slot argument into D2, installs the five MOB array bases, and calls legacy_moblist_unlink, which preserves the slot attribute bits", "48e73f3e", "bsr.w 0x8d00"),
    ...
    (0x8D86, 0x8D94, "legacy_moblist_link_slot_base13", "sort key word; slot index word", "void", "stack-to-register wrapper; branches to the shared body at 0x8DD2", "Loads the slot index into D7, adds the fixed base 0x0D, and enters the shared slot-index link body", "48e71f06", "addi.w 0xd, d7"),
    (0x8D94, 0x8D9E, "legacy_moblist_link_slot_base0", "sort key word; slot index word", "void", "stack-to-register wrapper; branches to the shared body at 0x8DD2", "Loads the slot index into D7 unmodified and enters the shared slot-index link body", "48e71f06", "bra.b 0x8dd2"),
    (0x8D9E, 0x8DAC, "legacy_moblist_link_slot_base17", "sort key word; slot index word", "void", "stack-to-register wrapper; branches to the shared body at 0x8DD2", "Loads the slot index into D7, adds the fixed base 0x11, and enters the shared slot-index link body", "48e71f06", "addi.w 0x11, d7"),
    (0x8DAC, 0x8DBA, "legacy_moblist_link_slot_base21", "sort key word; slot index word", "void", "stack-to-register wrapper; branches to the shared body at 0x8DD2", "Loads the slot index into D7, adds the fixed base 0x15, and enters the shared slot-index link body", "48e71f06", "addi.w 0x15, d7"),
    (0x8DBA, 0x8DC8, "legacy_moblist_link_slot_base25", "sort key word; slot index word", "void", "stack-to-register wrapper; branches to the shared body at 0x8DD2", "Loads the slot index into D7, adds the fixed base 0x19, and enters the shared slot-index link body", "48e71f06", "addi.w 0x19, d7"),
    (0x8DC8, 0x8DD2, "legacy_moblist_link_slot_base1", "sort key word; slot index word", "void", "stack-to-register wrapper; falls through into the shared body at 0x8DD2", "Loads the slot index into D7, adds one, and falls through into the shared slot-index link body", "48e71f06", "addq.w 0x1, d7"),
    (0x8DD2, 0x8E90, "legacy_moblist_link_by_slot_index", "D6 = sort key; D7 = biased slot index", "void", "shared register body reached only from the six 0x8D86-0x8DC8 wrappers", "Splices slot D7 into the depth chain sorted by D6: derives the 0x905F80 bucket index ((D6>>5)+1)*4 capped at 0x7E, walks the chain rewriting the neighbours' low ten-bit link fields, replaces every stale bucket head and the 0x90493A global head, then stores the neighbour indices and the key to (A5/A6/A0, 2*D7)", "3c2f0022", "move.w d6, (a0, d7.w)"),
    (0x8E90, 0x8F38, "legacy_moblist_remove_by_slot_index", "slot index word", "void", "normal stack ABI; module has no Gauntlet II caller", "Removes a slot addressed by index: increments and doubles it, repairs both neighbours' link fields through an 0xFC00 mask, rewrites any 0x905F80 bucket entry that referenced it, then zeroes the sort key at (A0,D2) and both link words at (A5,D2)/(A6,D2)", "48e73e06", "lea.l 0x904892"),
    ...
    (0x9252, 0x9284, "legacy_recursive_path_move_entry", "actor-list index word; step word; mode word", "D0 propagated from legacy_recursive_path_move", "normal stack ABI; module has no Gauntlet II caller", "Stack entry for the recursive path mover: doubles the actor index into D0, loads the step into D6 and the mode into D7, installs A2-A4, decrements the pending-move counter at 0x904ADC, and calls legacy_recursive_path_move", "48e73f3e", "subq.w 0x1, 0x904adc"),
    ...
    (0x9880, 0x989C, "legacy_test_actor_contact_b", "D1.w cell offset; D5.w actor-list offset", "D0 predicate and condition codes", "byte-identical 28-byte copy of 0x9864 around the same stale external target 0x49572", "Halves both offsets and invokes the legacy actor-contact predicate; the twenty-eight instruction bytes are identical to 0x9864", "3f013005", "jsr 0x49572"),
