...
ROWS = (
    (0x35C4, "process_coins", "Decode two packed four-channel coin-counter samples: per channel the delta is (current + 4 - previous) & 3, deltas above one are rejected, the surviving count is scaled by ((cfg[+0x0A] & 0x1C) >> 2) + 1, and the bonus thresholds at 0x6986 indexed by cfg[+0x0A] >> 5 convert accumulated coins into pending credits", "current packed 2-bit counter sample; previous packed 2-bit counter sample", "void", "normal two-slot stack ABI; only each slot's low byte is consumed", "4e56fffe48e73f38"),
