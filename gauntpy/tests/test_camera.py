...
def _active_player(state: GameState, index: int, slot: int, px: int, py: int) -> None:
    ...
    position as ``hpos >> 6`` / ``vpos >> 6``, so we store ``px << 6``
    etc.  The ``slot`` is used as both mob_slot and the index into the hpos
    array (identity-is-location convention, §1.2).
    """
    player = state.players[index]
    player.status = PlayerStatus.ALIVE_HERE
    player.mob_slot = slot
    state.mobs.hpos[slot] = px << 6
    state.mobs.vpos[slot] = py << 6
...
class TestSinglePlayerMovement:
    ...
    def test_scroll_y_steps_toward_target(self):
        """scroll_y increases by 2 when target is far below current (§17 step 3)."""
        state = _gameplay_state()
        # Player at pixel (200, 200):
        #   target_y = 0x1E8 - 200 - 0x6C = 488 - 200 - 108 = 180
        #   start scroll_y = 100 -> delta = 80 >= 3 -> scroll_y += 2 = 102
        state.scroll_x = 96            # pre-position X so we isolate Y
        state.scroll_y = 100
        _active_player(state, 0, slot=10, px=200, py=200)
        main_scroll_playfield(state)
        assert state.scroll_y == 102
...
class TestSnapToTarget:
    def test_snap_x_when_delta_is_one(self):
        ...
        state = _gameplay_state()
        # Player at (200, 200): target_x = 96, target_y = 180
    ...
    def test_snap_y_when_delta_is_minus_two(self):
        ...
        state = _gameplay_state()
        # Player at (200, 200): target_y = 180
        # Start scroll_y = 182 -> delta = -2 -> snap -> scroll_y = 180
        state.scroll_x = 96
        state.scroll_y = 182
        _active_player(state, 0, slot=10, px=200, py=200)
        main_scroll_playfield(state)
        assert state.scroll_y == 180
...
class TestRubberBand:
    def test_spread_clamped_to_200_px_horizontally(self):
        """When two players are 300 px apart in X, extent is clamped to 0xC8 (§17).
        ...
        # Pre-position scroll to the rubber-band target:
        # midpoint_x = (100+300)/2 = 200, target_x = 200 - 104 = 96
        # midpoint_y = (100+100)/2 = 100, target_y = 488 - 100 - 108 = 280
        state.scroll_x = 96
        state.scroll_y = 280
        main_scroll_playfield(state)
        # If rubber band: snap, stays at 96.  If no rubber band: steps away (target=146).
        assert state.scroll_x == 96

    def test_spread_clamped_to_200_px_vertically(self):
        """When two players are 300 px apart in Y, extent is clamped to 0xC8 (§17).

        Without rubber band: midpoint_y = (100+400)/2 = 250, target_y = 130.
        With rubber band:    max_y = 100+200 = 300, midpoint_y = 200, target_y = 180.
        """
        state = _gameplay_state()
        ...
        state.level_players_active = 2
        # target_x = 200 - 104 = 96
        # rubber-band target_y = 488 - 200 - 108 = 180
        state.scroll_x = 96
        state.scroll_y = 180
        main_scroll_playfield(state)
        assert state.scroll_y == 180
...
class TestScrollClamp:
    ...
    def test_clamp_to_minimum_y(self):
        ...
        state = _gameplay_state()
        # Player at py=500: target_y = 488 - 500 - 108 = -120.
        _active_player(state, 0, slot=10, px=100, py=500)
        state.scroll_x = 100
        state.scroll_y = 1
        main_scroll_playfield(state)
...
class TestStepSize:
    ...
    def test_x_step_is_exactly_two(self, start_x: int, expected_x: int):
        ...
        state.scroll_y = 180                       # pre-position Y at target
        main_scroll_playfield(state)
        assert state.scroll_x == expected_x

    @pytest.mark.parametrize("start_y,expected_y", [
        (100, 102),   # delta = +80 >= 3 -> step +2
        (200, 198),   # delta = -20 <= -3 -> step -2
    ])
...
class TestViewportConversion:
    ...
    def test_mid_maze_player_centres_exactly(self):
        ...
        assert (256 - vx, 256 - vy) == (120, 120)     # dead centre of the viewport

    def test_viewport_never_leaves_the_512px_maze(self):
        for px, py in [(0, 0), (512, 512), (256, 0), (0, 256), (448, 448)]:
            state = _gameplay_state()
            _active_player(state, 0, slot=500, px=px, py=py)
            snap_camera(state)
            vx, vy = viewport_scroll(state, 240, 240)
            assert 0 <= vx <= 512 - 240, (px, py, vx)
            assert 0 <= vy <= 512 - 240, (px, py, vy)

    def test_low_player_is_still_on_screen(self):
        """The inverted-Y register used to push a low player off-screen (the
    ...
    def test_inverted_y_uses_rom_bottom_clamp(self):
        """0x46F56 clamps the inverted register at 1, not the generic 5."""
        state = _gameplay_state()
        _active_player(state, 0, slot=500, px=256, py=448)
        snap_camera(state)
        assert state.scroll_y == 1
        assert viewport_scroll(state, 240, 240)[1] == 259

    def test_inverted_y_uses_rom_top_clamp(self):
        state = _gameplay_state()
...
class TestSnapCamera:
    def test_snap_jumps_straight_to_target(self):
        ...
        state.scroll_y = 0
        snap_camera(state)
        # ROM target: x = 200 - 0x68 = 96; y = 0x1E8 - 200 - 0x6C = 180.
        assert (state.scroll_x, state.scroll_y) == (96, 180)
...
class TestSeamWraparound:
    def test_x_position_past_seam_is_adjusted(self):
        ...
        Player A: x=5,  hpos = 5<<6 = 320
        Player B: x=520, hpos = 520<<6 = 33280 (raw pos, past the right edge)
