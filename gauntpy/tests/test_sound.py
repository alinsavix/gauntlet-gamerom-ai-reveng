...
so ``sound_log`` -- not ``sound_queue`` -- is the record of what was emitted.
...
def test_sound_play_sends_immediately_when_the_latch_accepts():
    ...
    state = GameState()
    assert state.sound_holdoff == 0, "the fast path needs a clear holdoff"

    sound.sound_play(state, 0x37)

    assert state.sound_log == [0x37], "the board got the byte this instant"
    assert state.sound_queue == [], "and it never touched the ring"
...
def test_a_busy_latch_falls_back_to_the_ring(monkeypatch):
    ...
    assert state.sound_queue == [0x37], "parked on the ring for the drain"
...
def test_the_busy_fallback_drops_on_a_full_ring_as_well(monkeypatch):
    ...
    assert state.sound_queue == [0x50 + i for i in range(sound.SOUND_QUEUE_CAPACITY)]
    assert (0x50 + sound.SOUND_QUEUE_CAPACITY) not in state.sound_queue
...
def test_enqueue_sound_is_the_ring_primitive_sound_play_falls_back_to():
    ...
    assert state.sound_queue == list(range(sound.SOUND_QUEUE_CAPACITY))
...
def test_sound_speech_play_is_silenced_by_the_operator_bit():
    ...
    assert state.sound_queue == [], "nor the ring"
...
def test_a_busy_latch_costs_an_attempt_but_leaves_the_read_head_alone(monkeypatch):
    ...
    drain."""
    state = GameState()
    state.sound_queue = [0x37, 0x38]
    monkeypatch.setattr(sound, "try_send_sound_command", lambda _s, _c: False)

    sound.main_update_sound(state)

    assert state.sound_queue == [0x37, 0x38], "nothing consumed while the latch was busy"
    assert state.sound_log == []
...
def test_a_failed_status_send_retries_next_frame_and_counts_the_attempt(monkeypatch):
    ...
    assert state.sound_retry_count == 1
    assert state.sound_holdoff == 0, "one failure is not yet a reset"
...
def test_the_retry_count_forces_a_full_reset_above_the_threshold(monkeypatch):
    ...
    state = GameState()
    state.sound_idle_timer = 0
    state.sound_retry_count = sound.SOUND_RETRY_LIMIT - 1
    state.sound_queue = [0x11]
    monkeypatch.setattr(sound, "send_status_query", lambda _s: False)

    sound.sound_response(state)          # count reaches exactly 0xB4
    assert state.sound_retry_count == sound.SOUND_RETRY_LIMIT
    assert state.sound_holdoff == 0, "0xB4 itself is not above 0xB4"

    state.sound_idle_timer = 0
    sound.sound_response(state)          # count passes 0xB4
    assert state.sound_holdoff == sound.SOUND_RESET_HOLDOFF
    assert state.sound_retry_count == 0, "the reset clears the count"
    assert state.sound_queue == [], "and the ring"
...
def test_reinitialize_takes_the_ordinary_path():
    ...
    held = GameState()
    held.sound_holdoff = 4
    sound.sound_play(held, sound.SOUND_REINITIALIZE)
    assert held.sound_queue == [0x00], "and rings up behind a holdoff, like anything else"

    held.sound_holdoff = 0
