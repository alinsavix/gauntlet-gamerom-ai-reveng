...
59. **Secret eligibility is copied at level setup.** Zeroing
    ...
    `secret_trick_id`, followed by the normal solo-party cancellation. A forced
    invitation needs a stable valid `secret_player`; disable the ordinary task so
...
86. **Host options use cabinet settings when one exists.** Reduce Text is
    `game_settings` bit 10. With nonnegative `game_mode`, it selects the
    alternate first-encounter bank before speech lookup; only record zero exists,
...
96. **Projectile ownership gates item side effects.** A monster or dragon shot
    can remove a destructible potion and play its break sound, but the
