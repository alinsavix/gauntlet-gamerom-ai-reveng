...
def player_add_score_with_mult(
    state: GameState, player_index: int, base_score: int,
) -> None:
    """0x5214C -- multiply unsigned score/bonus words and add to a longword.

    The caller's base is narrowed at 0x52158 before MULU.W at 0x52176.
    ADD.L at 0x52178 wraps the score; ORI.B at 0x52182 sets only redraw bit
    zero. No high-score check or other game-state writes occur here.
    """
    player = state.players[player_index]
    award = (base_score & 0xFFFF) * (player.bonusmult & 0xFFFF)
    player.score = (player.score + award) & 0xFFFF_FFFF
