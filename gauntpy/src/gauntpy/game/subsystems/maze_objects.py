...
from .player_transport import _tport_pos_table, tport_find_id as tport_find_id
...
def record_transporter_secret_progress(
    ...
    *,
    ...
    if state.secret_trick_id == 0x56:
        ...
        pad_count = len(_tport_pos_table(state))
        for pad in (source_slot, destination_pad):
            pad_id = tport_find_id(state, pad)
            # A lookup miss is count + 1, not a pad to mark.
            if pad_id <= pad_count:
                mask |= 1 << pad_id
