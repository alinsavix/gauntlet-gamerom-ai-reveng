...
from gex.palettes import palette_make_special
...
__all__ = ["MAZE_CELLS", "TERRAIN_TYPES", "PlayfieldCache", "build_playfield_image", "draw_playfield"]
...
def _dotat(img, xloc: int, yloc: int) -> None:
...
def _render_dots(img, xloc: int, yloc: int, count: int) -> None:
    for dx, dy in _DOT_POSITIONS.get(count, []):
        _dotat(img, xloc + dx, yloc + dy)
...
def build_playfield_image(maze: MazeLike):
    ...
    **Open question, flagged rather than silently resolved:** wall/door
    ...
    hardware-accurate.
    """
    img = blank_image(WORLD_PIXELS, WORLD_PIXELS)
    ...
    for y in range(MAZE_CELLS):
        for x in range(MAZE_CELLS):
            ...
            write_stamp_to_image(img, stamp, x * 16, y * 16)
    ...
    for y in range(MAZE_CELLS):
        for x in range(MAZE_CELLS):
            ...
            if stamp is not None:
                write_stamp_to_image(img, stamp, x * 16 + stamp.nudgex, y * 16 + stamp.nudgey)
            if dots:
                _render_dots(img, x * 16, y * 16, dots)
...
class PlayfieldCache:
    """The cached world raster plus the maze identity it was built from, so
    the compositor can tell when it needs rebuilding (a new level) versus
    ...
    maze_id: int
    image: object  # PIL.Image.Image, kept untyped here to avoid a hard PIL import at type-check time
...
def playfield_cache_for(maze: MazeLike, cache: PlayfieldCache | None) -> PlayfieldCache:
    ...
    if cache is not None and cache.maze_id == id(maze):
        return cache
    return PlayfieldCache(maze_id=id(maze), image=build_playfield_image(maze))
