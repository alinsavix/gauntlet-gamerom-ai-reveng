...
normal play the full record still appears, but its hold shortens from 150 to
120 frames. In attract modes the bit selects a separate short-record bank; only
its food entry is populated. Speech remains controlled by the independent
Disable Speech option. The caller learns whether speech played so it can pace
itself. Seen flags reset
